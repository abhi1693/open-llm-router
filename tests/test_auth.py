...
def test_v1_models_allows_when_auth_disabled(monkeypatch):
...
def test_v1_models_accepts_valid_api_key(monkeypatch):
    with _build_client(
        monkeypatch,
        INGRESS_AUTH_REQUIRED="true",
        INGRESS_API_KEYS="router-key-1,router-key-2",
    ) as client:
        response = client.get("/v1/models", headers={"Authorization": "Bearer router-key-2"})
        assert response.status_code == 200


def test_v1_models_accepts_valid_oauth_token(monkeypatch):
